...
def find_plate_number_region(img):
    ...
    for i in range(len(contours)):
        ...
        # 矩形的四个坐标（顺序不定，但是一定是一个左下角、左上角、右上角、右下角 这种循环顺序（开始是哪个点未知））
...
def cut(img_or_imgpath):
    """
    截取车牌区域
    :param img_or_imgpath:
    :return:
    """
    if isinstance(img_or_imgpath, str):
        img = cv2.imread(img_or_imgpath)
    else:
        img = img_or_imgpath
