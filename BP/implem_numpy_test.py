...
w = [0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65] #这个是 连接权值
# 偏置项b不进行更新
b = [0.35, 0.65] # 两层网络的偏执项
l = [5, 10] # 输入的一个样本
...
def f1(w, b, l):
    ...
    w[0] = w[0] - 0.5 * (t1 * w[6] + t2 * w[7]) * h1 * (1 - h1) * l[0]  # 根据公式弄清楚
...
for i in range(1001):
    r1, r2, w = f1(w, b, l)
    print("第{}次迭代后，结果值为:({},{}),权重更新为:{}".format(i, r1, r2, w))  # 每次迭代后的中间层的输出值
