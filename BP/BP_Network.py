...
sampleoutnorm += noise  #  (2, 20)
# print("sampleoutnorm.shape:", sampleoutnorm.shape)
# 超参数
maxepochs = 60000 #最大迭代次数
learnrate = 0.035  # 学习率
errorfinal = 0.65*10**(-3)  # 错误率
samnum = 20 # 样本数
indim = 3 #输入特征维度
outdim = 2 # 输出维度（）
hiddenunitnum = 8 # 隐藏层的神经元个数

# 网络设计
w1 = 0.5*np.random.rand(hiddenunitnum, indim) - 0.1  # 跟样本特征数和隐藏神经元个数有关
print("w1的shape:", w1.shape) # (8, 3)
...
for i in range(maxepochs): # 迭代次数
    ...
    # 错误
    err = sampleoutnorm - networkout  # (2, 20)
    sse = sum(sum(err**2))
    # print(sse)
    # 前面是计算误差， 后面更新w1 w2的值
