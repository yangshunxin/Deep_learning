...
# 手写数字识别的数据集主要包括三个部分： 训练集(5.5w, mnist.train)、测试集(1w, mnist.test)、验证集（0.5w, minist.validation）
...
# 输出维度大小
n_classes = train_label.shape[1] # 10

# 模型构建
# 1.设置数据输入的占位符
x = tf.placeholder(tf.float32, shape=[None, input_dim], name='x')
y = tf.placeholder(tf.float32, shape=[None, n_classes], name='y')
learn_rate = tf.placeholder(tf.float32, name='learn_rate') # 动态的学习率

def learn_rate_func(epoch):
     ''''
     根据迭代次数来，产生学习率
     '''
     return learning_rate_base * (0.9 ** int(epoch/10))

def get_variable(name, shape=None, dtype=tf.float32, initializer=tf.random_normal_initializer(mean=0, stddev=0.1)):
     '''
     返回一个对应的变量, 外面要用 variable_scope
     :param name:
     :param shape:
     :param dtype:
     :param initializer:
...
def le_net(x, y):
    ...
    with tf.variable_scope('conv2'):
        ...
        # input：输入数据，必须是一个四维格式的图像数据，具体格式和data_format有关，如果data_format是NHWC的时候，input的格式为: [batch_size, height, weight, channels] => [批次中的图片数目，图片的高度，图片的宽度，图片的通道数]；如果data_format是NCHW的时候，input的格式为: [batch_size, channels, height, weight] => [批次中的图片数目，图片的通道数，图片的高度，图片的宽度]
        # filter：卷积核，是一个4维格式的数据，shape：[height, weight, in_channels, out_channels] => [窗口的高度，窗口的宽度，输入的channel通道数（上一层图片的深度）， 输出的通道数（卷积核数目）]
        # strides：步长，是一个4维的数据，每一维数据必须核data_format格式匹配，表示的是在data_format上 每一维的移动步长：[batch, in_height, in_weight, in_channels] => [样本上的移动大小，高度的移动大小，宽度的移动大小，深度的移动大小],要求在样本上和在深度通道上的移动必须是1；
